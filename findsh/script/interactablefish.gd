...
@onready var original_albedo = $LargeFish.get_active_material(0).albedo_color
...
func _physics_process(delta: float) -> void:
	if FishInfo.pressed_fish != fish_name:
		$LargeFish.get_active_material(0).albedo_color = original_albedo
...
func _on_fish_pressed(camera: Node, event: InputEvent, event_position: Vector3, normal: Vector3, shape_idx: int) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			$LargeFish.get_active_material(0).albedo_color = Color("00ffff7e")
